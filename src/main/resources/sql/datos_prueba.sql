...
CREATE TABLE heroes (
                        uid INT AUTO_INCREMENT PRIMARY KEY,
                        nombre_heroe VARCHAR(100) NOT NULL UNIQUE,
                        inteligencia INT NOT NULL CHECK (inteligencia >= 0),
                        fuerza INT NOT NULL CHECK (fuerza >= 0),
                        agilidad INT NOT NULL CHECK (agilidad >= 0),
                        vida INT NOT NULL,
                        mana INT,
                        historia_heroe TEXT NOT NULL
);
CREATE TABLE objetos (
                         uid INT AUTO_INCREMENT PRIMARY KEY,
                         nombre_objeto VARCHAR(100) NOT NULL UNIQUE,
                         daño_fisico INT CHECK (daño_fisico >= 5),
                         daño_magico INT CHECK (daño_magico >= 5),
                         vida_adicional INT CHECK (vida_adicional >= 50),
                         mana_adicional INT CHECK (mana_adicional >= 50),
                         regeneracion_mana DECIMAL(5, 2) CHECK (regeneracion_mana >= 0.5),
                         regeneracion_vida DECIMAL(5, 2) CHECK (regeneracion_vida >= 0.5),
                         descripcion_objeto TEXT NOT NULL
);
CREATE TABLE heroe_objeto (
                              id_heroe INT NOT NULL,
                              id_objeto INT NOT NULL,
                              cantidad_objeto INT NOT NULL CHECK (cantidad_objeto > 0),
                              PRIMARY KEY (id_heroe, id_objeto),
                              FOREIGN KEY (id_heroe) REFERENCES heroes(uid) ON DELETE CASCADE,
                              FOREIGN KEY (id_objeto) REFERENCES objetos(uid) ON DELETE CASCADE
);
INSERT INTO Usuario (uid, username, password, rol) VALUES
...
INSERT INTO Objetos (uid, nombre_objeto, daño_fisico, daño_magico, vida_adicional, mana_adicional, regeneracion_mana, regeneracion_vida, descripcion_objeto) VALUES
                                                                                                                                                                 ...
                                                                                                                                                                 (2, 'Aghanim\'s Scepter', NULL, NULL, 200, 300, 1.0, 0.5, 'Mejora habilidades y añade nuevas capacidades a héroes específicos.'),
...
(13, 'Heaven\'s Halberd', NULL, NULL, 200, NULL, NULL, NULL, 'Desactiva ataques básicos de un enemigo por unos segundos.'),
